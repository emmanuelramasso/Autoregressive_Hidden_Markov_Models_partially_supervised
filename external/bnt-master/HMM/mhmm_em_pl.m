function [LL, prior, transmat, mu, Sigma, mixmat] = ...
     mhmm_em_pl(data, prior, transmat, mu, Sigma, mixmat, pl, varargin);
...
while (num_iter <= max_iter) & ~converged
  ...
  [loglik, exp_num_trans, exp_num_visits1, postmix, m, ip, op] = ...
      ess_mhmm(prior, transmat, mixmat, mu, Sigma, data, pl);
  ...
  if adj_mix
    mixmatnew = mk_stochastic(postmix);
  end
  if adj_mu | adj_Sigma
    [mu2, Sigma2] = mixgauss_Mstep(postmix, m, op, ip, 'cov_type', cov_type);
    if adj_mu
      munew = reshape(mu2, [O Q M]);
    end
    if adj_Sigma
      Sigmanew = reshape(Sigma2, [O O Q M]);
    end
  end
  ...
  if ~converged
      ...
      if adj_mix, mixmat = mixmatnew; end
...
for ex=1:numex
  ...
  if mix
    ...
    B = B .* pl{ex};
    if 0
        B2 = B2 .* permute(repmat(pl{ex},[1,1,M]),[1,3,2]); % B2(i,k,t) = Pr(y(t) | Q(t)=i, M(t)=k)
        [~, ~, gamma,  current_loglik, xi, gamma2] = ...
            fwdback(prior, transmat, B, 'obslik2', B2, 'mixmat', mixmat);
    else
        [~, ~, gamma,  current_loglik, xi, C, gamma2] = fwdback_phmm_mix(prior, transmat, B', pl{ex}', permute(B2,[3,1,2]), mixmat);
        gamma = gamma'; gamma2 = permute(gamma2,[2,3,1]); %gamma2(j,k,t) = p(Q(t)=j, M(t)=k | y(1:T)) (only for MOG  outputs)
    end
  else
    ...
    if 0
        ...
        [~, ~, gamma,  current_loglik, xi] = fwdback_phmm_mix(prior, transmat, B', pl{ex}');
        gamma = gamma';
    end
  end   
