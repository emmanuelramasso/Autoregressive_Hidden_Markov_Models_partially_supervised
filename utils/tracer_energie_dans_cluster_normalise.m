function hdle = tracer_energie_dans_cluster_normalise(clusters, energie, T)
...
% l=ceil(length(u)/r);
c=(cumsum(energie)); 
hold on, plot(T,100*c/sum(energie),'m','LineWidth',4)
...
for i=1:length(u)
   ...
   plot(T, (100*((cumsum(E))./cumsum(energie))),s{u(i)},'Color',Z(u(i),:),'LineWidth',2,'MarkerSize',5)
