...
addpath '/home/emmanuel.ramasso/Documents/Dropbox'/8_PROGS/developpements/Progs/PROJETS/ARPHMM/
addpath '/home/emmanuel.ramasso/Documents/Dropbox'/8_PROGS/developpements/Progs/PROJETS/ARPHMM/PHMM/
addpath '/home/emmanuel.ramasso/Documents/Dropbox'/8_PROGS/developpements/Progs/PROJETS/ARPHMM/PHMM/utils
addpath '/home/emmanuel.ramasso/Documents/Dropbox'/8_PROGS/developpements/Progs/PROJETS/ARPHMM/utils/
addpath '/home/emmanuel.ramasso/Documents/Dropbox'/8_PROGS/developpements/Progs/SOFTLABELS/ARPHMM/

clear all
%load '/home/ramassoemmanuel/Documents/Dropbox/6_BENCHMARK/Biocomposites/TestsSeptembre2015/DossierDonneesTraiteesParSylvain/biocomp_procSylvain_OP1E2/Descripteurs_0P1E2_ihmprocsylvain_deplinclu.mat'
% load '/home/ramassoemmanuel/Documents/Dropbox/6_BENCHMARK/Biocomposites/TestsSeptembre2015/DossierDonneesTraiteesParSylvain/biocomp_procSylvain_OP1E2/Hits0P1E2.mat'
load '/home/emmanuel.ramasso/Documents/Dropbox/6_BENCHMARK'/biocomp2/P1_vide_2_version2/carac.mat
load '/home/emmanuel.ramasso/Documents/Dropbox/6_BENCHMARK'/biocomp2/P1_vide_2_version2/lessalves.mat

% affichage des salves
if 0
    idx = find(carac(:,8) ==2); % Je récupère la voie 1
    T = carac(:,5); % On récupère le temps
    figure, hold on;
    for i = 1:length(idx)
        t = T(idx(i)):1/(5e6):T(idx(i))+(length(lessalves{idx(i)})-1)/5e6; % Le début de la salve i est T(i) et chaque point est séparé de 1/5e6 secondes
        plot(t,lessalves{idx(i)}), drawnow
        [i,length(idx)]
    end
end
...
if 1
    ...
    for i=1:length(lessalves),
        S=[S; lessalves{i}/max(lessalves{i})];
    end
    lessalves = S; clear S
end
...
for i=1:length(lessalves)
    [a b]=max(lessalves{i});
    lessalves{i} = lessalves{i}(max(1,b-nbAvant+1):min(b+nbApres,length(lessalves{i})));
    lessalves{i} = lessalves{i}(:);
end

% figure, for i=1:length(lessalves), clf, plot(lessalves{i}), i, pause(1), end

% numero des salves choisies pour apprendre les modeles
% referencesSalves=[26 60 130] % MC2 LOC
% referencesSalves=[595] % P1_2vide
referencesSalves=[424 595 1000 1500] % P3_4

%referencesSalves=[742 1914 2820 8000 8200 8548 9042] %MC1 LOC

salvesModeles = lessalves(referencesSalves);
...
for lsa=1:length(salvesModeles)%    48;
    ...
    figure, hold on, 
    qualitymf_supZ = -inf; % si loglik
    %qualitymf_supZ = inf; % si mse ou ndei
    
    for l=1:5 
        [gamma2_sup, NDEI_sup, qualitymf_sup, signalt_sup, ...
            signalcomb_sup, B_sup, sigma_sup, A_sup, Pi_sup, ~,~, ~, ~, ~, LL_sup,~,~,USE_CRISP_GAMMA] =...
            AUTOREGRESSIVE(y, pl0,Q,predc);
    ...
    models(k).T = length(y);
    models(k).my = my;
    models(k).USE_CRISP_GAMMA = USE_CRISP_GAMMA;
    models(k).stdy = stdy;
    models(k).NDEI = NDEI_sup;
    models(k).qualitymf = qualitymf_sup;
    models(k).LL = LL_sup(end);
    models(k).LLnorm = LL_sup(end)/models(k).T;
    models(k).usePriorGamma = usePriorGamma;
    models(k).useGtruthRand = useGtruthRand;
    
    k=k+1;
    close all
end

% test la reconnaissance des salves apprises => donne ref des mesures de nouveauté
[lesreponsesLOGL_train, lesreponsesMSE_train, ...
    ...
    appliquerModelesSurSalves(models, lessalves(referencesSalves));
...
[lesreponsesLOGL, lesreponsesMSE, ...
    ...
    appliquerModelesSurSalves(models, lessalves);
toc

figure
subplot(321),hold on,plot(lesreponsesL2RAT,'o'),title('L2RAT')
for i=1:length(referencesSalves), plot(referencesSalves(i)*ones(1,size(lesreponsesL2RAT,2)),lesreponsesL2RAT(referencesSalves(i),:),'rx'), end
subplot(322),hold on,plot(lesreponsesLOGL,'o'),title('LOGL')
for i=1:length(referencesSalves), plot(referencesSalves(i)*ones(1,size(lesreponsesLOGL,2)),lesreponsesLOGL(referencesSalves(i),:),'rx'), end
subplot(323),hold on,plot(lesreponsesMAXERR,'o'),title('MAXE')
for i=1:length(referencesSalves), plot(referencesSalves(i)*ones(1,size(lesreponsesMAXERR,2)),lesreponsesMAXERR(referencesSalves(i),:),'rx'), end
subplot(324),hold on,plot(lesreponsesNDEI,'o'),title('NDEI')
for i=1:length(referencesSalves), plot(referencesSalves(i)*ones(1,size(lesreponsesNDEI,2)),lesreponsesNDEI(referencesSalves(i),:),'rx'), end
subplot(325),hold on,plot(lesreponsesPSNR,'o'),title('PSNR')
for i=1:length(referencesSalves), plot(referencesSalves(i)*ones(1,size(lesreponsesPSNR,2)),lesreponsesPSNR(referencesSalves(i),:),'rx'), end
subplot(326),hold on,plot(lesreponsesMSE,'o'),title('MSE')
for i=1:length(referencesSalves), plot(referencesSalves(i)*ones(1,size(lesreponsesMSE,2)),lesreponsesMSE(referencesSalves(i),:),'rx'), end


X=[lesreponsesL2RAT lesreponsesLOGL lesreponsesMAXERR lesreponsesNDEI lesreponsesPSNR lesreponsesMSE];% defcarac];

if 0
    dataidxnotok=find(isinf(sum(X,2)) | isnan(sum(X,2)));
    dataidxall = 1:size(X,1);
    dataidxok = dataidxall; dataidxok(dataidxnotok)=[];
    X = X(dataidxok,:);
    disp(sprintf('%%%%\n Il y a %d NAN ou INF',length(dataidxnotok)))

else
    
    c=tabulate(carac(:,8)); 
    for i=1:size(c,1)
        g=find(carac(:,8)==c(i,1));
        Z=X(g,:);
        dataidxnotok=find(isinf(Z) | isnan(Z));
        disp(sprintf('%%%%\n Il y a %d NAN ou INF (total, not line) => replace par extrap!',length(dataidxnotok)))
        dataidxnotok=find(isinf(Z));
        Z(dataidxnotok) = nan;
        Z = fillmissing(Z,'linear','SamplePoints',carac(g,5));
        X(g,:)=Z;
    end
...
% CAS 6
% load '/home/emmanuel.ramasso/Documents/Dropbox/6_BENCHMARK/biocomp2'/P1_vide_2_version2/carac.mat
% carac(:,[1:4 6 7 9:end])=nan;
% ??carac=carac(dataidxok,:);
% carac(:,[13 14 15 19 21 22])=X;

% CAS 12
load '/home/emmanuel.ramasso/Documents/Dropbox/6_BENCHMARK/biocomp2'/P1_vide_2_version2/carac.mat
carac(:,[1:4 6 7 9:end])=nan;
% ??carac=carac(dataidxok,:);
carac(:,[13 14 15 19 21 22 23:28])=X;


% load '/home/emmanuel.ramasso/Documents/Dropbox/6_BENCHMARK/biocomp2'/P1_vide_2_version2/carac.mat
v=6*ones(size(carac,1),1); v(dataidxok)=vc;
v=vc;
densites_amplitude(v,carac,13,1)
duree_vs_amplitude(vc,carac,carac(:,5),6,13,12);
duree_vs_amplitude(v,carac,carac(:,5),6,13,12);
tracer_energie_dans_cluster(v,carac(:,22),carac(:,5))
tracer_energie_dans_cluster(v,carac(:,11),carac(:,5))

[coeff, score, latent, tsquared, explained] = pca(X);%diff(X,2));
[LoadingsPM,specVarPM,TPM,stats,score] = factoran(diff(X), 3,'rotate','promax');

% [coeff, score, latent, tsquared, explained] = pca((X));
% [LoadingsPM,specVarPM,TPM,stats,score] = factoran((X), 3,'rotate','promax');

figure,plot(score(:,1),score(:,2),'.'), hold on,
W=50;
for i=W+1:W:size(X,1),
    plot(score(i-W:i,1),score(i-W:i,2),'ro'),
    title(sprintf('%d/%d (%d/100)',i,size(X,1),round(100*i/size(X,1)))),
    if i>2*W
        plot(score(i-W-W:i-W,1),score(i-W-W:i-W,2),'g.'),
        title(sprintf('%d/%d (%d/100)',i,size(X,1),round(100*i/size(X,1)))),
    end
    drawnow,
end

figure,plot3(score(:,1),score(:,2),score(:,3),'.'), hold on,
W=50;
for i=W+1:W:size(X,1),
    plot3(score(i-W:i,1), score(i-W:i,2),score(i-W:i,3),'ro'),
    title(sprintf('%d/%d (%d/100)',i,size(X,1),round(100*i/size(X,1)))),
    if i>2*W
        plot3(score(i-W-W:i-W,1),score(i-W-W:i-W,2),score(i-W-W:i-W,3),'g.'),
        title(sprintf('%d/%d (%d/100)',i,size(X,1),round(100*i/size(X,1)))),
    end
    drawnow,
end
...
figure
subplot(321),hold on,plot(diff(medfilt1(X(:,1),9,'omitnan')),'o'),title('L2RAT')
subplot(322),hold on,plot(diff(medfilt1(X(:,2),9,'omitnan')),'o'),title('LOGL')
subplot(323),hold on,plot(diff(medfilt1(X(:,3),9,'omitnan')),'o'),title('MAXE')
subplot(324),hold on,plot(diff(medfilt1(X(:,4),9,'omitnan')),'o'),title('NDEI')
subplot(325),hold on,plot(diff(medfilt1(X(:,5),9,'omitnan')),'o'),title('PSNR')
subplot(326),hold on,plot(diff(medfilt1(X(:,6),9,'omitnan')),'o'),title('MSE')
